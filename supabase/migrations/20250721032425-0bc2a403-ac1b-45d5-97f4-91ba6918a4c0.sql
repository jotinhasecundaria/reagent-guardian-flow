...
-- Verificar e inserir unidades padrão
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM public.units WHERE name = 'Lab Central') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.manufacturers WHERE name = 'BioTech Labs') THEN
        INSERT INTO public.manufacturers (id, name, contact_info) VALUES 
        ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'BioTech Labs', '{"phone": "(11) 2222-3333", "email": "contato@biotech.com"}'),
        ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'DiagCorp', '{"phone": "(11) 2222-3334", "email": "vendas@diagcorp.com"}'),
        ('cccccccc-cccc-cccc-cccc-cccccccccccc', 'LabMax', '{"phone": "(11) 2222-3335", "email": "suporte@labmax.com"}'),
        ('dddddddd-dddd-dddd-dddd-dddddddddddd', 'ReagentPlus', '{"phone": "(11) 2222-3336", "email": "info@reagentplus.com"}');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM public.reagents WHERE name = 'Glicose Oxidase') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.exam_types WHERE name = 'Glicemia de Jejum') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.reagent_lots WHERE lot_number = 'LOT2024001') THEN
